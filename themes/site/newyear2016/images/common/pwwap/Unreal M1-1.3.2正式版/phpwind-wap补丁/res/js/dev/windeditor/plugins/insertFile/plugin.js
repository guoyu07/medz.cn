	...
	var	dialog = $('<div class="edit_menu" style="display:none;">\
				<div class="edit_menu_file">\
					<div class="edit_menu_top">\
						...
						<strong>附件上传</strong>\
						<span class="edit_tips" style="left:80px;" title="可上传格式和大小:"></span>\
					</div>\
					<div class="edit_menu_cont">\
						<div class="edit_uping">\
							<span class="num">附件上传数量限制<em id="J_num">'+ ATTACH_CONFIG.attachnum +'</em>个</span>\
							<input class="btn btn_submit" id="J_buttonPlaceHolder" type="button" value="批量上传" />\
						</div>\
		...
		var editorDoc = _self.editorDoc = _self.iframe[0].contentWindow.document,
			...
			plugin_icon.on('click',function(e) {
				...
				if(!$.contains(_self.container[0],dialog[0]) ) {
					...
					Wind.js(plupload_root + 'plupload.full.min.js?v='+ GV.JS_VERSION,
                            plupload_root + 'zh_CN.js?v=' + GV.JS_VERSION, function() {
                        //实例化一个plupload上传对象
                        uploader = new plupload.Uploader({
                            runtimes : 'html5,flash,html4',
                            browse_button : 'J_buttonPlaceHolder', //触发文件选择对话框的按钮，为那个元素id
                            url : ATTACH_CONFIG.uploadUrl+'&_json=1', //服务器端的上传页面地址
                            flash_swf_url : plupload_root+'Moxie.swf', //swf文件，当需要使用swf方式进行上传时需要配置该参数
                            filters: {
			...
			dialog.on('click','a.J_del_queue',function(e) {
				e.preventDefault();
				var dd = $(this).parent().parent();
                var file = uploader.getFile(dd.attr('id'));
            ...
            function fileQueued(uploader, files) {
            	for (var i = 0, len = files.length; i < len; i++) {
            		...
            		name = file.name, file_extension = name.substring(name.lastIndexOf('.') + 1, name.length).toLowerCase();
            		...
            		if (allowSize && file.size > allowSize) {
                        ...
                        invalid = true;
            		} else if (file_list_box.find('dd').size() > allowFileCount) {
            			var tip = '上传数量超出限制';
                        invalid = true;
            		}
        			//如果是无效的则取消上传
