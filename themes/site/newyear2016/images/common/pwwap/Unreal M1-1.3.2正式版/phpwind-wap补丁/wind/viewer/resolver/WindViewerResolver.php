...
class WindViewerResolver extends WindNormalViewerResolver implements IWindViewerResolver {
	...
	public function compile($template, $suffix = '', $readOnly = false, $forceOutput = false) {
		...
		if($template==='TPL:common.layout_error'){
            self::$isFirst = true;
        }
        if(self::$isFirst){
            if($_GET['mobile'] == 'yes') 
            {
                $isMobile = true;
				WindCookie::delete('useMobile');
            } elseif (stripos($_SERVER['HTTP_HOST'], 'm.') === 0 || stripos($_SERVER['SERVER_NAME'],'m.') === 0)
            {
                $isMobile = true;
            } else
            {
                $isMobile = stristr($_SERVER['HTTP_USER_AGENT'], 'mobile'); //判断是否手机设置
            }
        
            $useMobile = WindCookie::get('useMobile') !== 'no'; //判断是否禁用wap主题
            if ($isMobile && $useMobile)
            {
                $a = explode('.', $template);
                $a[count($a) - 1] = 'wap.' . $a[count($a) - 1];
                $wapTemplate = implode('.', $a);
                list($wapTemplateFile, $wapCompileFile, $this->currentThemeKey) = $this->windView->getViewTemplate($wapTemplate, $suffix);
                if (is_file($wapTemplateFile)) {
                    $templateFile = $wapTemplateFile;
                    $compileFile = $wapCompileFile;
                }
            }
